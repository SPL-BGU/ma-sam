...
(:action switch_on
	...
	:precondition (and (power_avail ?s) (on_board ?i ?s))
...
(:action calibrate
	...
	:precondition (and (calibration_target ?i ?d) (on_board ?i ?s) (power_on ?i) (pointing ?s ?d))
	:effect       (and (calibrated ?i)
))

(:action take_image
	:parameters   (?s - satellite ?i - instrument ?d - direction ?m - mode)
	:precondition (and (supports ?i ?m) (on_board ?i ?s) (calibrated ?i) (pointing ?s ?d) (power_on ?i))
